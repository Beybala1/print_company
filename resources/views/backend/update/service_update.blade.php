    ...
    <div class="col-xl">
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">Servisi-i redaktə et</h5>
            </div>
            <div class="card-body">
            <form action="{{ route('service.update',[$service_edit->id]) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <img width="100%" height="350" class="mb-3" src="{{ url($service_edit->image) }}">
                <div class="mb-3">
                    <label class="form-label" for="basic-default-fullname">Başlıq</label>
                    <input type="text" name="title" value="{{ $service_edit->title }}" class="form-control" id="basic-default-fullname"
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" id="text" class="form-control" rows="10"
                        placeholder="Açıqlama" required>{{ $service_edit->description }}</textarea>
                </div>
                ...
                <div class="pt-4">
                    ...
                    <a href="{{ route('service.index') }}" class="btn btn-label-secondary">Ləğv et</a>
                </div>
            </form>
        </div>
    </div>
